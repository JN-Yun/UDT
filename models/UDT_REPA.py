...
# ToMe: https://github.com/dbolya/tomesd
# --------------------------------------------------------
import torch
...
class UDT(nn.Module):
    def __init__(
        ...
        z_dims=[768],        # REPA-related
        projector_dim=2048,  # REPA-related
        **block_kwargs
    ):
        super().__init__()
    ...
    def forward(self, x, t, y, z=None):
        ...
        for i in range(self.enc_depth):
            ...
            # 2. REPA
            if (i + 1) == self.enc_depth:
                if self.training:
                    x_full_res = x.clone()
                    for restore_fn in reversed(unmerge_stack):
                        x_full_res = restore_fn(x_full_res)            
                    xs = [projector(x_full_res.reshape(-1, D)).reshape(B, x_full_res.shape[1], -1) for projector in self.projectors]
