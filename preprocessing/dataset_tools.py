...
def open_image_folder(source_dir, *, max_images: Optional[int]) -> tuple[int, Iterator[ImageEntry]]:
    ...
    if len(labels) == 0:
        ...
        if len(toplevel_indices) > 1:
            ...
            single_label = next(iter(toplevel_indices))  # Get the only key from toplevel_indices
            labels = {arch_fname: toplevel_indices[single_label] for arch_fname in arch_fnames.values()}
...
def open_image_zip(source, *, max_images: Optional[int]) -> tuple[int, Iterator[ImageEntry]]:
    with zipfile.ZipFile(source, mode='r') as z:
        ...
        # input_images = [f for f in sorted(z.namelist()) if f.startswith('ILSVRC/Data/CLS-LOC/train/') and is_image_ext(f)]  # JN_change (for only train directoy from zip)
...
def encode(
    ...
    vae = StabilityVAEEncoder(vae_name=model_url, batch_size=512)
    num_files, input_iter = open_dataset(source, max_images=max_images)
    archive_root_dir, save_bytes, close_dest = open_dest(dest)
    labels = []
